...
if (isset($_POST['edit_publish'])) {
    ...
    // Build the query
    $update_query = "UPDATE login_user SET name='$name', email_address='$email_address'";
...
<div class="mt-24 px-6 flex gap-6">
  ...
  <div class="bg-white/95 backdrop-blur-lg rounded-3xl lg:rounded-[3rem] shadow-2xl lg:shadow-2xl p-1 sm:p-3 lg:p-8 border border-gray-200/50 flex flex-col h-screen lg:h-[650px] items-center relative overflow-hidden">
    ...
    <div id="sidebarContent" class="flex flex-col w-full h-full pt-4 lg:pt-6 transition-all duration-500 ease-out">
      ...
      <!-- Menu - NO SCROLLBAR on desktop + All content visible -->
      <nav class="w-full flex-1 px-2 sm:px-3 space-y-1.5 lg:space-y-3 overflow-y-auto lg:overflow-visible scrollbar-thin scrollbar-thumb-gray-300/70 scrollbar-track-transparent lg:max-h-none max-h-[calc(100vh-240px)] lg:max-h-none">
        ...
        <a href="department_management.php" 
           ...
           <i class="fas fa-building text-gray-600 w-3.5 h-3.5 sm:w-4 sm:h-4 flex-shrink-0 shadow-sm rounded p-0.5 bg-white/50"></i>
           <span class="font-semibold tracking-wide flex-1 min-w-0 truncate text-dark-800">Offices</span>
        </a>

        <!-- Employees - Active style -->
        <a href="view_user.php" 
           ...
           <i class="fas fa-users text-blue-500 group-hover:text-indigo-600 w-3.5 h-3.5 sm:w-4 sm:h-4 flex-shrink-0 transition-all duration-300"></i>
    ...
    <div class="flex flex-col lg:flex-row lg:justify-between lg:items-center mb-4 gap-4 lg:gap-0">
      ...
      <div class="flex flex-wrap items-center gap-2 lg:gap-4 order-1 lg:order-2 justify-center lg:justify-end">
        ...
        <button onclick="$('#modalAddAdmin').removeClass('hidden');" 
                class="bg-gradient-to-r from-green-600 to-green-500 text-white px-3 sm:px-5 py-2 rounded-xl hover:scale-105 hover:shadow-lg flex items-center gap-2 transition-all duration-300 text-sm sm:text-base whitespace-nowrap">
          <i class="fas fa-user-plus"></i> Add User
        </button>

        <!-- Archived Admins -->
        <button onclick="$('#modalArchivedAdmins').removeClass('hidden');" 
                class="bg-gradient-to-r from-yellow-500 to-yellow-400 text-white px-3 sm:px-5 py-2 rounded-xl hover:scale-105 hover:shadow-lg flex items-center gap-2 transition-all duration-300 text-sm sm:text-base whitespace-nowrap">
          <i class="fas fa-archive"></i> View Archived User
        </button>
    ...
    <div class="max-h-[400px] lg:max-h-[450px] overflow-y-auto overflow-x-auto lg:overflow-x-hidden">
      <table id="table3" class="w-full border border-gray-200 min-w-full lg:min-w-0">
          <thead class="bg-green-700 text-white sticky top-0 z-10">
            <tr>
              <th class="px-2 sm:px-4 py-2 text-xs sm:text-sm">Full Name</th>
              <th class="px-2 sm:px-4 py-2 text-xs sm:text-sm">Email Address</th>
              <th class="px-2 sm:px-4 py-2 text-xs sm:text-sm text-center">Verification</th>
        ...
        WHERE user_status != 'Archived'";
          ...
          ?>
            <tr class="hover:bg-gray-50 transition-colors">
              <td class="px-2 sm:px-4 py-3 text-xs sm:text-sm"><?php echo htmlspecialchars($rs['name']); ?></td>
              <td class="px-2 sm:px-4 py-3 text-xs sm:text-sm"><?php echo htmlspecialchars($rs['email_address']); ?></td>
              <td class="px-2 sm:px-4 py-3 text-center">
                <?php if($rs['otp_verified'] == 1){ ?>
                ...
                <div class="relative flex justify-center">
                  ...
                  <div id="menu<?php echo $rs['id']; ?>" 
                    ...
                    <a href="view_user.php?id=<?php echo $rs['id']; ?>"
                       onclick="closeMenu(<?php echo $rs['id']; ?>)"
                       class="flex items-center gap-2 w-full px-3 py-2 hover:bg-gray-100 text-blue-600 text-sm">
                      <i class="fas fa-edit"></i> Edit
                    </a>

                    <!-- ARCHIVE -->
                    <button onclick="confirmArchiveAdmin(<?php echo $rs['status']; ?>); closeMenu(<?php echo $rs['id']; ?>)"
                    ...
                    <button type="button"
                      onclick="openOtpModal(
                        <?php echo htmlspecialchars(json_encode($rs['email_address']), ENT_QUOTES, 'UTF-8'); ?>, 
                        <?php echo (int)$rs['otp_verified']; ?>
                      ); closeMenu(<?php echo $rs['id']; ?>)"
                      class="flex items-center gap-2 w-full px-3 py-2 hover:bg-gray-100 text-yellow-600 text-sm">
...
<div id="modalAddAdmin" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-30 backdrop-blur-sm">
  ...
  <div class="bg-white/90 backdrop-blur-lg rounded-2xl shadow-2xl w-[90%] max-w-md p-6 relative animate-fadeIn">
    ...
    <form action="create_user.php" method="POST" class="flex flex-col gap-4">
      <input type="hidden" name="user_status" value="Employee">
...
<button type="button"
onclick="togglePassword('user_password','toggleIconAdd')"
...
<?php 
if($edit_id != ''){
    $q = mysqli_query($conn,"SELECT * FROM login_user WHERE id='$edit_id'");
...
<div id="modalEditAdmin" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-30 backdrop-blur-sm">
  ...
  <form method="POST" class="flex flex-col gap-4">
      ...
      <input type="email" name="email_address" value="<?php echo htmlspecialchars($rs['email_address']); ?>" class="border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none transition" required>
...
<button type="button"
onclick="togglePassword('edit_user_password','toggleIconEdit')"
class="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-600 hover:text-gray-800">
  ...
  <div class="bg-white/90 backdrop-blur-lg rounded-2xl shadow-2xl w-11/12 max-w-4xl p-6 relative animate-fadeIn">
    ...
    <div class="overflow-x-auto max-h-[60vh]">
      <table class="min-w-full border border-gray-200">
        ...
        <?php
          $archived = mysqli_query($conn,"SELECT * FROM login_user WHERE user_status='Archived'");
          while($a = mysqli_fetch_assoc($archived)){
        ?>
          <tr class="border-b hover:bg-gray-50">
            <td class="px-4 py-2"><?php echo htmlspecialchars($a['name']); ?></td>
            <td class="px-4 py-2"><?php echo htmlspecialchars($a['email_address']); ?></td>
...
function confirmArchiveAdmin(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location = "view_user.php?archive_id=" + id;
        }
    });
}
</script>
...
function confirmUnarchiveAdmin(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location = "view_user.php?unarchive_id=" + id;
        }
    });
}
</script>
